...
description: 요구사항 발견, 디버깅, UI/Figma 구현이 한 요청에 섞여 있을 때 사용합니다. 단일 분류를 강제하지 않고 여러 워크플로우 모듈을 조합하며, 레포 루트 tmp/에 임시 작업 문서를 남기고 질문 순서를 조율합니다.
---
...
사용자가 다른 위치를 명시하지 않으면 레포 루트의 `tmp/` 디렉터리에 작업 문서를 작성합니다.
...
```text
tmp/YYYYMMDD-HHMM-<type>-<topic>-workflow.md
```
